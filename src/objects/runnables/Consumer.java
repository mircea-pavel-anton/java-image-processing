...
public class Consumer implements Runnable {
	 ...
	 * The FSM models a nested for loop, that executes one iteration per
	 * extraction from the BlockingQueue
	 * 
	 * Since this is the first state, we also need to initailize the pixel in our
	 * matrix.
	 * In this step, we read the first of 3 (or 4) bytes that make up our pixel data
	 * This first byte represents the inensity of the blue color
	 * As such, we create a new pixel, and set the blue channel = character
	 */
	private void FSM_State1() { bitmapImage.setPixelAt(x, y, new Pixel(character)); }
	 ...
	 * value for the green color
	 * No further actions are required, other than setting the green color channel
	 * of our pixel = character
	 */
	private void FSM_State2() { bitmapImage.getPixelAt(x, y).setGreen(character); }

	/** The third state of the modelled FSM that handles parsing the Raw Pixel Data
	 * The FSM models a nested for loop, that executes one iteration per
	 * extraction from the BlockingQueue
	 * 
	 * The third state handles the third byte, which represents the intensity
	 * value for the red color, so we set the pixel's red channel value to be
	 * equal to character
	 * 
	 * Other than that, if the image we are reading has 24 bits-per-pixel
	 * (3 bytes per pixel), we need to increment our matrix coordinates
	 * in order to move to the next pixel
	 */
	private void FSM_State3() {
		bitmapImage.getPixelAt(x, y).setRed(character);
		if (bitmapImage.getBytesPerPixel() == 3) increment();
	}
	 ...
	 * value for the alpha channel, so we set the pixel's alpha value to be
	 * equal to character
	 * 
	 * This state is only used in the case of a 32-bit BMP file
	 * If we reach this state, we need to increment our matrix coordinates
	 * to move to the next pixel, since state 3 did not pass the condition
	 * for incrementing
	 */
	...
	/** Handles matrix coordinate incrementation, based on the modelled
	 * nested for loop:
	 ...
	 *     }
	 * }
	 * isDone keeps track wheter or not the loop has finished all its
	 * iterations
	 */
	...
	/** Handles parsing the bits from the BMP header, based on their index 
	 * See this medium.com article for more details on header format:
	...
	*/
	...
	/** Handles parsing the bits from the DIB header, based on their index 
	 * See this medium.com article for more details on header format:
	 * https://medium.com/sysf/bits-to-bitmaps-a-simple-walkthrough-of-bmp-image-format-765dc6857393
	*/
	...
	/** Handles parsing the bits from the RPD, based on their index 
	 * See this medium.com article for more details on header format:
	 * https://medium.com/sysf/bits-to-bitmaps-a-simple-walkthrough-of-bmp-image-format-765dc6857393
	*/
	private void readRawPixelData() throws Exception {
		// init pixel matrix if needed
		if (bitmapImage.getPixels() == null) bitmapImage.initPixels();
		...
		switch (bitmapImage.getBytesPerPixel()) {
			case 3: // 24 bit BMP
				switch ( position % 3) {
					case 1: FSM_State1(); break; // read blue
					case 2: FSM_State2(); break; // read green
					case 0: FSM_State3(); break; // read red & increment
				}
				if (isDone) return; // if we run out of pixels
				break;
			case 4:
				switch ( position % 4) {
					case 1: FSM_State1(); break; // read blue
					case 2: FSM_State2(); break; // read green
					case 3: FSM_State3(); break; // read red
					case 4: FSM_State4(); break; // read alpha & increment
				}
				if (isDone) return; // if we run out of pixels
				break;
