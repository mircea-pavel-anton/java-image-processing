...
public class Timer {
	 ...
	 */
	private final HashMap<String, Long> jobStart;

	/**
	 * This hashmap will store a key-value pair, the key being the name of the job
	 * and the value being the time in millis at which it was ended
	 */
	private final HashMap<String, Long> jobEnd;

	/**
	 * Adds a key-value pair into the jobStart hashmap, associating the current time
	 * in millis to the job identifier
	 * 
	 * @param identifier = the identifier of the job to end
	 * @throws Exception if thee job was never started to begin with
	 */
	public void startJob(final String identifier) throws Exception {
	 ...
	 */
	public long stopJob(final String identifier) throws Exception {
		endJob(identifier);
		final long duration = getJobDuration(identifier);
		deleteJobRecord(identifier);
		return duration;
	}

	/**
	 * Adds a key-value pair into the jobEnd hashmap, associating the current time
	 * in millis to the job identifier
	 * 
	 * @param identifier = the identifier of the job to end
	 * @throws Exception if thee job was never started to begin with
	 */
	private void endJob(final String identifier) throws Exception {
		if (!jobStart.containsKey(identifier)) {
	 ...
	 */
	private void deleteJobRecord(final String identifier) {
		jobStart.remove(identifier);
		jobEnd.remove(identifier);
	}

	/**
	 * Will calculate the duration of the job by subtracting jobEnd[identifier] -
	 * jobStart[identifier]
	 * 
	 * @param identifier = the identifier of the job
	 * @return the job duration, in milliseconds
	 * @throws Exception if the job wasn't even started, or if it was started but
	 *                   never ended
	 */
	private long getJobDuration(final String identifier) throws Exception {
		if ( !jobStart.containsKey(identifier) ) {
			throw new Exception("No such job is running");
		}
