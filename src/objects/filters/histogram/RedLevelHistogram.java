...
public class RedLevelHistogram extends AbstractHistogramFilter {
	 ...
	 */
	public RedLevelHistogram(final int width, final int height, final int samples) {
		super(samples, true, "red");
	...
	public Image filter(final Image image) {
		...
		final int maxValue = Arrays.stream(redLevelHistogram).max().getAsInt();
		...
		for (int x = 0; x < width; x++) {
			for (int y = 0; y < height; y++) {
				if (y < redLevelHistogram[(int) (x / xSize)] / ySize) {
